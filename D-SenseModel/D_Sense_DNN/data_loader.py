...
''' 
    ...
    Others
    'Cross-location-Orientation',   # Task: Cross-location Orientation recognition
    'Cross-user-Orientation',       # Task: Cross-user Orientation recognition
    'Cross-gesture-Orientation',    # Task: Cross-gesture Orientation recognition
    'Cross-orientation-Location',   # Task: Cross-orientation Location recognition
    'Cross-gesture-Location',       # Task: Cross-gesture Location recognition
    'Cross-user-Location',          # Task: Cross-user Location recognition
...
class load_data:
    def __init__(self,
        ...
        self.task_map = {
            ...
            'Cross-location-Orientation': self.Cross_location_Orientation,
            'Cross-user-Orientation': self.Cross_user_Orientation,
            'Cross-gesture-Orientation': self.Cross_gesture_Orientation,
            'Cross-orientation-Location': self.Cross_orientation_Location,
            'Cross-gesture-Location': self.Cross_gesture_Location,
            'Cross-user-Location': self.Cross_user_Location
        }
    ...
    def Cross_location_Orientation(self):
        data, _, orientation_label, location_label, T_max = self.single_task_data()
        test_mask = location_label == self.domain
        Tasks = ['Orientation']
        return {
            'data_train': data[~test_mask],
            'data_test': data[test_mask],
            'Orientation_label_train': orientation_label[~test_mask],
            'Orientation_label_test': orientation_label[test_mask]
        }, T_max,  Tasks

    def Cross_user_Orientation(self):
        data, _, user_label, orientation_label, _, T_max = self.extract_data()
        test_mask = user_label == self.domain
        Tasks = ['Orientation']
        return {
            'data_train': data[~test_mask],
            'data_test': data[test_mask],
    ...
    def Cross_gesture_Orientation(self):
        data, gesture_label, orientation_label, _, T_max = self.single_task_data()
        test_mask = gesture_label == self.domain
        Tasks = ['Orientation']
        return {
            'data_train': data[~test_mask],
            'data_test': data[test_mask],
    ...
    def Cross_orientation_Location(self):
        data, _, orientation_label, location_label, T_max = self.single_task_data()
        test_mask = orientation_label == self.domain
        Tasks = ['Location']
        return {
            'data_train': data[~test_mask],
            'data_test': data[test_mask],
            'Location_label_train': location_label[~test_mask],
            'Location_label_test': location_label[test_mask]
        }, T_max, Tasks

    def Cross_gesture_Location(self):
        data, gesture_label, _, location_label, T_max = self.single_task_data()
        test_mask = gesture_label == self.domain
        Tasks = ['Location']
        return {
